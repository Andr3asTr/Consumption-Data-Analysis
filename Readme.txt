...
The data included are measurements per second so they are a lot. The analyst could therefore use the software to sum up the values or find for example the mean value of power consumption for a specific time period.
The reports that the software will export, based on what the analyst wants, can be in .txt (text file), .md (markdown) or .html(website).
